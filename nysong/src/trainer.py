...
class TorchTrainer:
    ...
    def train(
        ...
        Returns:
            loss and accuracy
        """
        wandb.init(project="automl", reinit=True)
        best_test_acc = -1.0
        best_test_f1 = -1.0
        ...
        for epoch in range(n_epoch):
            ...
            wandb.run.name = str(trial_num)
            wandb.log({"test acc":test_acc, "test loss": test_loss, "test f1 score":test_f1, "MACS" : calc_macs(self.model, tuple(data.shape[1:]))})
