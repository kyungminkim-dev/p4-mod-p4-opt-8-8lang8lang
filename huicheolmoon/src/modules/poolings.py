...
class MaxPoolGenerator(GeneratorAbstract):
    ...
    def __call__(self, repeat: int = 1):
        # ceil_mode argument를 받기 위해 기존의 assign에서 변경 (*self.args -> self.args[0], self.args[1], ceil_mode=self.args[2])
        module = (
            [self.base_module(self.args[0], self.args[1], ceil_mode=self.args[2]) for _ in range(repeat)]
            if repeat > 1
            else self.base_module(self.args[0], self.args[1], ceil_mode=self.args[2])
        )
